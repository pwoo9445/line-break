...
q0 = [-3;2;pi/4;0;0;0;0;0]; % Arbitrary start position
q0(6)=q0(5)*tan(q0(3));
Q(:,1)=q0;
q = q0;
...
for i=2:N
    ...
    if abs(q(3) - thetaf) > 0.01
        
        if thetaf == pi/2
             dV = [(q(1)-xf);0;0;0]; % Minima

        elseif thetaf == 3*pi/2
            dV = [(q(1)-xf);0;0;0];
        else
            dV = [-tan(thetaf)*((q(2)-yf)-tan(thetaf)*(q(1)-xf));((q(2)-yf)-tan(thetaf)*(q(1)-xf));0;0];
        end
    else
        ...
        elseif thetaf == 3*pi/2
            ...
            dV = [-tan(thetaf+pi/2)*((q(2)-yf)-tan(thetaf+pi/2)*(q(1)-xf));((q(2)-yf)-tan(thetaf+pi/2)*(q(1)-xf));0;0];
        end
    ...
    %     % Base lambda
    %     L=[-r*q(7)*sin(q(3))*q(8); r*(q(7))*cos(q(3))*q(8)];
    %
    ...
    qdot = [q(5) ; q(6) ; q(7) ; q(8); Pd*(-dV+Fd+Qstar)-Lambda];%+[0;0;0;0;Qstar];
